...
try:    # try to open the file
    ...
    finalresult['ATTACK/NORMAL'] = saved_model.predict(resultinputdata)    # predict the result
    finalresult['ATTACK/NORMAL'].replace(to_replace=0, value='NORMAL', inplace=True)    # replace the 0 values with NORMAL
    finalresult['ATTACK/NORMAL'].replace(to_replace=1, value='ATTACK', inplace=True)    # replace the 1 values with ATTACK
    finalresult.to_csv("FINALRESULT.csv", index=False)  # save the result
